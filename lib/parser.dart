...
String audioPath         = String.fromEnvironment( "AUDIO_PATH",     defaultValue : "app/audio/" );
String fileScriptPath    = String.fromEnvironment( "FILE_SCRIPT",    defaultValue : "main.script" );
String fileVariablesPath = String.fromEnvironment( "FILE_VARIABLES", defaultValue : "default_variables.js" );
...
void preprocessLine() {
    if ( formattedLine.startsWith( "ifdef " ) ) {
        ...
        for (
            var wordIndex = 0;
            wordIndex < formattedWords.length;
            wordIndex++
        ) {
            for (
                var matchCounter = 0;
                matchCounter < formattedWords[ wordIndex ]
                    .allMatches( "\"" )
                    .length;
                matchCounter++
            ) {
            ...
            if ( isNotText ) {
                final word = formattedWords[ wordIndex ]
                    .replaceFirst( ";", "" )
                    .trim();
    ...
    if ( codeBracketCount > 0 ) {
        ...
        if ( preprocessedLine.startsWith( "define " ) ) {
            ...
            final line = preprocessedLine
                ...
                .split( "=" );
            final key = line.first.trim();
            final value = line.last
                .replaceRange( line.last.lastIndexOf( ";" ), null, "" )
                .trim();
            log( "key: \"$key\"", 2 );
            log( "value: \"$value\"", 2 );
            ...
            preprocessedLine = preprocessedLine.replaceRange( preprocessedLine.lastIndexOf( "{" ), null, "" )
                .replaceFirst( "label ", "" )
                .trim();
            log( "value: \"$preprocessedLine\"", 2 );

            fileApp.writeAsStringSync( "async function $preprocessedLine( _resolve ) {", mode : FileMode.writeOnlyAppend );
            ...
            fileApp.writeAsStringSync( preprocessedLine.trim(), mode : FileMode.writeOnlyAppend );
            ...
            fileApp.writeAsStringSync( "$preprocessedLine( _resolve );", mode : FileMode.writeOnlyAppend );
            ...
            fileApp.writeAsStringSync( "_Say( $preprocessedLine );", mode : FileMode.writeOnlyAppend );
            ...
            fileApp.writeAsStringSync( "_SayEx( $preprocessedLine );", mode : FileMode.writeOnlyAppend );
            ...
            final filename = words
                ...
                .join( "_" );
            var appearance = words
                .skipWhile( ( word ) => ( word != "with" ) )
                .toList()
                ..remove( "with" );
            ...
            ( preprocessedLine.startsWith( "play " ) ) ||
            ...
            fileApp.writeAsStringSync( "$function( MediaType.$type$file );", mode : FileMode.writeOnlyAppend );
