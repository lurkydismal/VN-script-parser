...
void main( List< String > arguments ) {
    ...
    argumentParser
        ..addOption(
            ...
            callback : ( String? level )
                => script_parser.debugLevel = (
                    int.tryParse( level ?? "null" )
                    ?? script_parser.debugLevel
                ),
            allowed : [ "1", "2", "3", "4" ],
        )
    ...
    FileSystemEntity.isDirectory( script_parser.fileScript.path ).then(
        ( inputFileIsDirectory ) {
            if ( inputFileIsDirectory ) {
                                ...
                                switch ( symbol ) {
                                    ...
                                    case ";":
                                    {
                                        ...
                                        if ( script_parser.debugLevel >= 5 ) {
                                            print( "${ script_parser.lineCounter }: formattedText: \"${ script_parser.formattedLine } \"" );
                                        }
                ...
                script_parser.fileVariables.writeAsStringSync( "", mode : FileMode.writeOnlyAppend );
